...
void printArray(long *values, int my_rank, int size) {
    ...
    for (i = 0; i < size; i++) {
        sprintf(msg, "Process %d should read: %ld bytes\n", i, values[i]);
...
struct BTreeNode* processTasks(Task *task, SubTask *subTask, struct BTreeNode *avl, Item **wordsList, long *wordsListSize, long *size, int *rank) {
    int currentTask;
    for (currentTask = 0; currentTask < task -> size; currentTask++) {
        *subTask = task -> subTasks[currentTask];
        ...
        avl = countWords(avl, subTask, *rank);
...
struct BTreeNode* receiveDataFromClients(struct BTreeNode *avl, int num_processes, Item **wordsList, long *wordsSize, MPI_Datatype itemType, MPI_Request *requests) {
